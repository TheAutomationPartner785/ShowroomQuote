...
const BottomActionBar = ({ onCancel, onPrevious, onNext, nextDisabled, currentStep }) => {
  ...
  return (
    <Box position="fixed" bottom="0" left="0" right="0" h="72px" bg="white" borderTop="1px solid var(--color-border)" boxShadow="0 -2px 8px rgba(0, 0, 0, 0.05)" zIndex="10">
      <Flex h="full" px="24px" gap="16px">
        <Button flex="1" h="56px" bg="white" border="2px solid var(--color-text-primary)" color="var(--color-text-primary)" fontSize="16px" fontWeight="700" borderRadius="var(--radius-md)" _hover={{ bg: 'var(--color-bg-subtle)' }} display="flex" gap="8px" onClick={handlePrevClick}>{currentStep === 1 ? <X size={20} /> : <ArrowLeft size={20} />}{getLeftButtonLabel()}</Button>

        <Button flex="1" h="56px" bg={nextDisabled ? 'var(--color-primary-disabled)' : 'var(--color-primary)'} color="white" fontSize="16px" fontWeight="700" borderRadius="var(--radius-md)" _hover={nextDisabled ? {} : { bg: 'var(--color-primary-hover)' }} disabled={nextDisabled} cursor={nextDisabled ? 'not-allowed' : 'pointer'} display="flex" gap="8px" onClick={handleNextClick}>{getRightButtonLabel()} {currentStep === 4 ? <Home size={20} /> : <ArrowRight size={20} />}</Button>
      </Flex>
